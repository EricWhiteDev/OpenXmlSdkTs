...
(function (root) {
  ...
  function defineOpenXml(Enumerable, Ltxml, XEnumerable) {
    ...
    function openFlatOpcFromXDoc(pkg, doc) {
      ...
      pkg.ctXDoc = new XDocument(
        ...
        new XElement(
          ...
          new XElement(CT.Default, new XAttribute("Extension", "rels"), new XAttribute("ContentType", openXml.contentTypes.relationships)),
      ...
      root.elements(FLATOPC.part).forEach(function (p) {
        ...
        var partType = "xml";
        if (!openXml.util.endsWith(contentType, "xml")) partType = "base64";
        if (partType === "xml") {
          var newPart = new openXml.OpenXmlPart(pkg, uri, contentType, partType, new XDocument(p.element(FLATOPC.xmlData).elements().firstOrDefault()));
          pkg.parts[uri] = newPart;
          if (contentType !== "application/vnd.openxmlformats-package.relationships+xml") {
            pkg.ctXDoc.getRoot().add(new XElement(CT.Override, new XAttribute("PartName", uri), new XAttribute("ContentType", contentType)));
          }
        }
        if (partType === "base64") {
      ...
      for (var part in that.parts) {
        ...
        if (thisPart !== null) {
          if (part === "[Content_Types].xml") {
            ...
            if (cte === null) {
              var extension = part.substring(part.lastIndexOf(".") + 1).toLowerCase();
              var dct = that.ctXDoc
                .getRoot()
                .elements(CT.Default)
                .firstOrDefault(function (e) {
    ...
    openXml.OpenXmlPackage.prototype.saveToFlatOpc = function () {
      ...
      var nsa = new XAttribute(XNamespace.xmlns + "pkg", "http://schemas.microsoft.com/office/2006/xmlPackage");
      var pkg = new XElement(FLATOPC._package, nsa);
      var flatOpc = new XDocument(xdec, new XProcessingInstruction("mso-application", 'progid="Word.Document"'), pkg);
      ...
      var intervalId = setInterval(function () {
        if (count < partCount) {
          for (var i = 0; i < 10; i++) {
            ...
            var ct = null;
            if (cte === null) {
              var extension = thisPart.uri.substring(thisPart.uri.lastIndexOf(".") + 1).toLowerCase();
              var dct = docpkg.ctXDoc
                .getRoot()
                .elements(CT.Default)
                .firstOrDefault(function (e) {
                  return e.attribute("Extension").value.toLowerCase() === extension;
    ...
    openXml.OpenXmlPackage.prototype.addPart = function (uri, contentType, partType, data) {
      ...
      this.ctXDoc.getRoot().add(new XElement(CT.Override, new XAttribute("PartName", uri), new XAttribute("ContentType", contentType)));
    ...
    function getRelationshipsFromRelsXml(pkg, part, relationshipPart) {
      ...
      var rels = rxDoc
        ...
        .select(function (r) {
          ...
          var theRel = new openXml.OpenXmlRelationship(pkg, part, r.attribute("Id").value, r.attribute("Type").value, r.attribute("Target").value, targetMode);
          return theRel;
        })
        .toArray();
      return rels;
    ...
    openXml.OpenXmlPackage.prototype.getRelationshipsByRelationshipType = function (relationshipType) {
      ...
      var rels = rxDoc
        ...
        .select(function (r) {
          ...
          var theRel = new openXml.OpenXmlRelationship(that, null, r.attribute("Id").value, relationshipType, r.attribute("Target").value, targetMode);
          return theRel;
        })
        .toArray();
      return rels;
    ...
    openXml.OpenXmlPackage.prototype.getRelationshipById = function (rId) {
      var rel = Enumerable.from(this.getRelationships()).firstOrDefault(function (r) {
    ...
    function addRelationshipToRelPart(part, relationshipId, relationshipType, target, targetMode) {
      ...
      var tm = null;
      if (targetMode !== "Internal") tm = new XAttribute("TargetMode", "External");
      rxDoc
        .getRoot()
        .add(
          new XElement(
    ...
    openXml.OpenXmlPackage.prototype.addRelationship = function (relationshipId, relationshipType, target, targetMode) {
      ...
      if (!rootRelationshipPart) {
        rootRelationshipsPart = this.addPart(
          "/_rels/.rels",
          this.contentTypes.relationships,
          "xml",
          new XDocument(new XElement(PKGREL.Relationships, new XAttribute("xmlns", pkgRelNs.namespaceName))),
        );
      }
    ...
    openXml.OpenXmlPackage.prototype.getContentType = function (uri) {
      ...
      ct = this.ctXDoc.descendants(CT.Override).firstOrDefault(function (o) {
        return o.attribute("PartName").value === uri;
      });
      if (ct === null) {
        var exti = uri.lastIndexOf(".");
        var ext = uri.substring(exti + 1);
        var dct = this.ctXDoc.descendants(CT.Default).firstOrDefault(function (d) {
    ...
    openXml.OpenXmlPart.prototype.addRelationship = function (relationshipId, relationshipType, target, targetMode) {
      ...
      if (!relsPart) {
        ...
        relsPart = this.pkg.addPart(
          relsPartUri,
          openXml.contentTypes.relationships,
          "xml",
          new XDocument(new XElement(PKGREL.Relationships, new XAttribute(XNamespace.xmlns + "rel", pkgRelNs.namespaceName))),
        );
      }
    ...
    bucketTimer.init = function () {
      ...
      for (prop in bucketTimer) {
        if (typeof bucketTimer[prop] !== "function" && prop !== "currentTime" && prop !== "currentBucket" && prop !== "beginningTime") {
    ...
    bucketTimer.dump = function () {
      ...
      for (prop in bucketTimer) {
        if (typeof bucketTimer[prop] !== "function" && prop !== "currentTime" && prop !== "currentBucket" && prop !== "beginningTime") {
          a.push(
            openXml.util.rPad(prop.toString(), 50, "-") +
              ": " +
    ...
    function convertToEntities(text) {
      ...
      for (i = 0; i < len; i += 1) {
        c = text.charCodeAt(i);
        if (c === 0xf0b7 || c === 0xf0a7 || c === 0xf076 || c === 0xf0d8 || c === 0xf0a8 || c === 0xf0fc || c === 0xf0e0 || c === 0xf0b2) {
  ...
  if (typeof define === typeof function () {} && define.amd) {
    // AMD
    define("openxml", ["linq", "ltxml", "ltxml-extensions"], function (Enumerable, Ltxml, XEnumerable) {
